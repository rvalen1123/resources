#!/usr/bin/env bash
# Installs custom skills into ~/.claude/skills/.
...
SRC_DIR="${RESOURCES_ROOT}/skills/templates"
SRC_DIR="${SRC_DIR%/}"
DEST_DIR="$CLAUDE_HOME/skills"

log "install-skills: $SRC_DIR -> $DEST_DIR"

if [[ ! -d "$SRC_DIR" ]]; then
  warn "no skills templates yet: $SRC_DIR — skipping"
  exit 0
fi

# Capture the file list up-front so a `find` failure (missing path, permissions)
# is caught here rather than silently emptying the loop. Using `<<< "$files"` to
# walk keeps the loop in the current shell so rc_overall mutations survive.
